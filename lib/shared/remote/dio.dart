...
class DioHelper{
 static Dio? dio;
  ...
  static Future<Response> getData({@required url, @required Map<String, dynamic>? query, token, Map<String, dynamic>? data})async{
      dio!.options.headers = {
        'Accept':'application/json',
 ...
 static Future<Response> postWithImageData({@required url, formdata,@required Map<String, dynamic>? query, token, @required Map<String, dynamic>? data})async{
   dio!.options.headers = {
     'Accept':'application/json',
     'Content-Type': 'multipart/form-data',
     'Authorization': 'Bearer $token',
